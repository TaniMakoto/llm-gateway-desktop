    ...
    fn extract_auth(&self, provider: &Provider) -> Option<AuthInfo> {
        ...
        match provider_type {
            ProviderType::GeminiCli => {
                ...
                //
                // LLM Gateway Desktop does not currently exchange the refresh_token for
...
mod tests {
    ...
    fn test_extract_auth_anthropic_auth_token_uses_claude_auth_strategy() {
        ...
        assert_eq!(auth.api_key, "test-anthropic-key");
    ...
    fn test_extract_auth_both_env_vars_prefer_auth_token() {
        ...
        let provider = create_provider(json!({
            "env": {
                "ANTHROPIC_BASE_URL": "https://api.anthropic.com",
                "ANTHROPIC_AUTH_TOKEN": "test-from-auth-token",
                "ANTHROPIC_API_KEY": "test-from-api-key"
            }
        }));

        let auth = adapter.extract_auth(&provider).unwrap();
        assert_eq!(auth.api_key, "test-from-auth-token");
        assert_eq!(auth.strategy, AuthStrategy::ClaudeAuth);
    }
