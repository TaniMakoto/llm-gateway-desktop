...
/// catalog with a top-level `models` field.  Return the llm-gateway–managed model
...
/// llm-gateway–owned `model_catalog_json`, using the same path ownership rules as
...
pub async fn handle_models(
    ...
    let models = crate::gateway::openai_models_response(state.db.as_ref())
...
async fn handle_messages_for_app(
    ...
    let mut ctx =
        RequestContext::new(&state, &body, &headers, app_type.clone(), tag, app_type_str, Some("anthropic")).await?;
...
/// 明文 JSON 重新生成正确的头。
pub(crate) fn decode_codex_request_body(
...
fn codex_proxy_error_json(
    ...
    let message = if upstream_status == Some(413) {
        ...
        // 避免「以为是 LLM Gateway Desktop 封装了 nginx / 是本地代理的锅」这种反复出现的误解。
        format!(
            concat!(
                ...
                "provider's server-side limit, not a LLM Gateway Desktop limit. ",
...
fn codex_proxy_error_code(error: &ProxyError) -> &'static str {
    match error {
        ...
        ProxyError::ProviderUnhealthy(_) => "llm_gateway_provider_unhealthy",
        ProxyError::ConfigError(_) => "llm_gateway_config_error",
        ProxyError::TransformError(_) => "llm_gateway_transform_error",
        ProxyError::InvalidRequest(_) => "llm_gateway_invalid_request",
        ProxyError::AuthError(_) => "llm_gateway_auth_error",
        ProxyError::UpstreamError { .. } => "llm_gateway_upstream_error",
        ProxyError::DatabaseError(_) => "llm_gateway_database_error",
        ProxyError::Internal(_) => "llm_gateway_internal_error",
        ProxyError::AlreadyRunning
        | ProxyError::NotRunning
        | ProxyError::BindFailed(_)
        | ProxyError::StopTimeout
        | ProxyError::StopFailed(_) => "llm_gateway_proxy_error",
    }
}
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    fn codex_proxy_forward_error_includes_context_and_cause() {
        ...
        assert!(message.contains("LLM Gateway Desktop local proxy failed"));
        ...
        assert_eq!(body["error"]["code"], "llm_gateway_forward_failed");
