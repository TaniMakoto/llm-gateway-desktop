...
pub fn apply_model_mapping(
    ...
    // Unified Gateway exact alias mapping. This runs before the legacy Claude
    // family mapper so arbitrary aliases such as `best-code` remain deterministic.
    if let Some(original) = original_model.as_deref() {
        if let Some(mapped) = provider
            .settings_config
            .get("gateway_model_map")
            .and_then(|value| value.get(original))
            .and_then(|value| value.as_str())
            .filter(|value| !value.trim().is_empty())
        {
            if mapped != original {
                log::debug!("[GatewayModelMapper] 模型映射: {original} → {mapped}");
                body["model"] = serde_json::json!(mapped);
                return (body, Some(original.to_string()), Some(mapped.to_string()));
            }
        }
    }
