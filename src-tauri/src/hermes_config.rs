...
///
/// Input (from LLM Gateway Desktop UI / database):
/// ```json
...
/// `providers:` dict. LLM Gateway Desktop treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by LLM Gateway Desktop)
...
/// keeping LLM Gateway Desktop free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay LLM Gateway Desktop must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a LLM Gateway Desktop edit to an unrelated field would silently
...
// LLM Gateway Desktop fills that gap by reading/writing the whole file as a markdown
...
mod tests {
    ...
    ///
    /// Saves and restores `LLM_GATEWAY_TEST_HOME` to avoid interfering with
    /// parallel tests in other modules.
    fn with_test_home<T>(test_fn: impl FnOnce() -> T) -> T {
        let _guard = test_guard();
        let tmp = tempfile::tempdir().unwrap();
        let old_test_home = std::env::var_os("LLM_GATEWAY_TEST_HOME");
        std::env::set_var("LLM_GATEWAY_TEST_HOME", tmp.path());
        ...
        match old_test_home {
            Some(value) => std::env::set_var("LLM_GATEWAY_TEST_HOME", value),
            None => std::env::remove_var("LLM_GATEWAY_TEST_HOME"),
        }
        result
    }
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later LLM Gateway Desktop edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a LLM Gateway Desktop toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
