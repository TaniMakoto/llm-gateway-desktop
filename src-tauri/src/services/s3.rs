...
mod tests {
    ...
    fn sig_v4_signing_against_aws_test_vector() {
        ...
        let creds = S3Credentials {
            access_key_id: "TEST_AWS_ACCESS_KEY_ID".to_string(),
        ...
        assert!(
            auth.starts_with(
                "AWS4-HMAC-SHA256 Credential=TEST_AWS_ACCESS_KEY_ID/20130524/us-east-1/s3/aws4_request"
            ),
...
mod integration_tests {
    ...
    async fn live_s3_put_get_head_roundtrip() {
        crate::proxy::http_client::init(None).ok();
        let creds = test_creds();
        let key = "llm-gateway-sync/v2/default/_integration_test.json";
        ...
        // 404
        let r = get_object(&creds, "llm-gateway-sync/_no_such_key", 1024).await;
        assert!(r.is_ok());
