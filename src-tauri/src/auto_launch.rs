...
/// 将 `/path/to/LLM Gateway Desktop.app/Contents/MacOS/LLM Gateway Desktop` 转换为 `/path/to/LLM Gateway Desktop.app`
#[cfg(target_os = "macos")]
...
fn get_auto_launch() -> Result<AutoLaunch, AppError> {
    let app_name = "LLM Gateway Desktop";
    let exe_path =
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        let exe_path = std::path::Path::new("/Applications/LLM Gateway Desktop.app/Contents/MacOS/LLM Gateway Desktop");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from("/Applications/LLM Gateway Desktop.app"))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        let exe_path =
            std::path::Path::new("/Users/test/My Apps/LLM Gateway Desktop.app/Contents/MacOS/LLM Gateway Desktop");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
    ...
    fn test_get_macos_app_bundle_path_not_in_bundle() {
        let exe_path = std::path::Path::new("/usr/local/bin/llm-gateway");
        let result = get_macos_app_bundle_path(exe_path);
    ...
    fn test_get_macos_app_bundle_path_dev_build() {
        ...
        let exe_path = std::path::Path::new("/Users/dev/project/target/debug/llm-gateway");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(result, None);
    }
}
