...
|---|---|---|---|
| 已完成 | 同协议保真/候选级转换 | Chat 候选直接转发；跨协议候选各自从原始请求构造，工具/reasoning/扩展字段有真实 HTTP 回归 | 继续扩展字段矩阵，不恢复全局 Chat→Responses 往返 |
...
| P2 | 更细的健康/配额状态 | 已区分源凭据冷却、公开模型冷却和物化协议熔断，并有排队及四策略；sub2api 另有错误率和 TTFT EWMA | 只有出现真实调度需求时再加延迟/错误率策略 |
...
先做客户端兼容档案和声明式参数规则，再补每供应商重试/超时、count_tokens、独立出站代理和更广的兼容矩阵。若确定 OAuth 账号池是近期核心需求，提前执行 CPA 原型，重新衡量继续自建这些能力的成本。无需为上述差距迁移到 sub2api。
